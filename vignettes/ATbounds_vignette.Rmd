...
The methodology used in the ATbounds package is described in detail in Lee and Weidner (2021), "Bounding Treatment Effects by Pooling Limited Information across Observations," <arXiv:??? [econ.EM]>.
...
@LaLonde-AER dataset available at [Rajeev Dehejia's Data Page](http://users.nber.org/~rdehejia/nswdata2.html).
...
```{r}
  Y <- RHC[,"survival"]
  D <- RHC[,"RHC"]
  X <- as.matrix(ATbounds::RHC[,-c(1,2)])
```
...
We now turn to our methods. We take the reference propensity score to be
...
None of the covariates in the observed sample
are identical. We therefore implement hierarchical, agglomerative
clustering with complete linkage, where the distance is measured by the
Euclidean distance after studentizing each of the covariates. To obtain
the clustered bound estimates, it is necessary to choose the number $m$
of clusters and the tuning parameter $Q$. We
consider $$m = \left\lceil \frac{n}{L} \right\rceil$$ for
$L = 10, 20$. In addition, we consider $Q = 3,4$.
